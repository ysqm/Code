...
{
    n = Read();m = Read();
    S = n*m+1;T = n*m+2;GraphSize = T; 
    for(int i = 1; i <= n ; ++i)
        for(int j = 1; j <= m ; ++j)
        {
            if(j != m) add_edge(Calc(i,j),Calc(i,j+1),1);
            //if(j != 1) add_edge(Calc(i,j),Calc(i,j-1),1);
            if(i != n) add_edge(Calc(i,j),Calc(i+1,j),1);
            //if(i != 1) add_edge(Calc(i,j),Calc(i-1,j),1);
            int x = Read();
            if(x == 1) add_edge(S,Calc(i,j),0x3f3f3f3f);
            else if(x == 2) add_edge(Calc(i,j),T,0x3f3f3f3f);
        }
}
