/*
    通过题目：
            洛谷UVA1402 Robotic Sort
            洛谷SP2059 CERC07S - Robotic Sort
*/
# include <ctime>
# include <cmath>
# include <queue>
...
void Build(const int L,const int R,int& p){int mid = (L+R)/2;sz(p = mid) = 1;mk(p) = false;if(L == R) return ;L < mid && (Build(L,mid-1,Son(p,0)),fa(Son(p,0)) = p);R > mid && (Build(mid+1,R,Son(p,1)),fa(Son(p,1)) = p);PushUp(p); }
