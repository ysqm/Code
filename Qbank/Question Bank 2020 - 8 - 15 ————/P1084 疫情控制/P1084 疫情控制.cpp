...
bool vis[N+5];
LL dis[Max_Step + 1][N + 5],need[N+5],deep[N+5];
...
struct node
{
	...
	node(const LL sum_=0,const int idx_=0):sum(sum_),idx(idx_){}
...
bool check(const int T)
{
	...
	for(reg int j = 1; j <= m ; ++j)
	{
		...
		if(f[0][cur] == 1 && dis[0][cur] <= di) Ar[++cnt] = node(di-dis[0][cur],cur);
		else vis[cur] = 1;
	}

	for(reg int i = head[1]; i ; i = e[i].Next)
		if(dfs_check(e[i].to) == 0)  need[++tot] = e[i].to;
		else vis[e[i].to] = 1;
	...
	for(reg int i = 1; i <= tot ; ++i)
		if(!vis[Ar[i].idx]) {vis[Ar[i].idx] = 1;Ar[i] = node(-1,0);}
	...
	for(reg int i = tot; i ; --i)
		if(!vis[need[i]])
		{
			if(Ar[cnt].sum >= dis[0][need[i]]) {--cnt;vis[need[i]] = 1;}
			else return false;
		}
