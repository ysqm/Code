...
int head[N + 42],e_cnt,n,m,s,deep[N + 42],son[N + 42],mod,dad[N + 42],siz[N + 42],top[N + 42],id[N + 42],from[N + 42],bh,base[N + 42];
...
{
	if(L == R){segtree[K] = base[from[L]] % mod;return ;}
	...
	segtree[K] = (segtree[Rs(K)] + segtree[Ls(K)]) % mod;
...
inline void lazy_down(const int L,const int R,const int K)
{
	...
	segtree[Rs(K)] = (segtree[Rs(K)] + lazy[K] * (R - mid)) % mod;//右区间[mid + 1,R] 
	
	segtree[Ls(K)] = (segtree[Ls(K)] + lazy[K] * (mid - L + 1)) % mod;//左区间[L,mid] 
	
	lazy[Ls(K)] = (lazy[K] + lazy[Ls(K)]) % mod;
	
	lazy[Rs(K)] = (lazy[Rs(K)] + lazy[K]) % mod;
...
{
	if(change_L <= L && change_R >= R){segtree[K] = (increment * (R - L + 1) + segtree[K]) % mod;lazy[K] = (increment + lazy[K]) % mod;return;}
...
inline LL ask_query(const int ask_L,const int ask_R,const int L,const int R,const int K)
{
	if(ask_L <= L && ask_R >= R) {
		...
		if(ask_R <= mid) return ask_query(ask_L,ask_R,L,mid,Ls(K)) % mod;
		
		else if(ask_L > mid) return ask_query(ask_L,ask_R,mid + 1,R,Rs(K)) % mod;
		
		else return (ask_query(ask_L,ask_R,L,mid,Ls(K)) + ask_query(ask_L,ask_R,mid + 1,R,Rs(K))) % mod;
	}
}
...
inline LL query_diff(int L,int R)
{
	...
	while(top[L] != top[R]){
		...
		res = (ask_query(id[top[L]],id[L],1,n,1) + res) % mod;
	...
	res = (ask_query(id[L],id[R],1,n,1) + res) % mod;
...
int main()
{
	...
	while(m--){
		...
		switch(pos){
			...
			case 3 :
					x = r();y = r();
					updata_change(id[x],id[x] + siz[x] - 1,y,1,n,1);
					break;
			case 4 :
					x = r();
