...
void Dp(const int& u,const int& fa)
{
	for(reg int i = head[u]; i ; i = e[i].next)
		if(e[i].to != fa)
		{
			...
			if(v > dp[u][0]){dp[u][1] = dp[u][0];dp[u][0] = v;mv[u] = i;}
   			
   			else if(v > dp[u][1]) dp[u][1] = v;
   			
   			l2 = Max(l2,dp[u][1] + dp[u][0]);
		}
}

int main()
{
	n = Read();m = Read();ans = (n-1)<<1;
	
	for(reg int i = 1; i < n ; ++i) add_edge(1,Read(),Read());
