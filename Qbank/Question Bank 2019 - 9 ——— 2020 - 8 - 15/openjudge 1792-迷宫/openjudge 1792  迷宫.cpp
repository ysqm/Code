...
# include <algorithm>
using namespace std;
void DFS(short x,short y,int c);
typedef struct{short x,y;} point;
bool flag;
short book[110][110];
char MAP[110][110];
int ans[11],n,k;
point Exit,Entry,NEXT[5]={{0,0},{0,1},{-1,0},{1,0},{0,-1}};
int main(){
	...
	for(int i=0;i<k;i++){
		...
		if(Entry.x==Exit.x&&Entry.y==Exit.y){
			flag=true;
		}else if(MAP[Entry.x][Entry.y]=='#'||MAP[Exit.x][Exit.y]=='#'){
			continue;
		}
		DFS(Entry.x,Entry.y,i);
		if(flag) printf("YES\n");
		else printf("NO\n");
	}
	return 0;
