...
queue <int> q;
int dis[N + 42],opposite_dis[N + 42],n,m,ans,s;
bool inq[N + 42];
...
int main()
{
	...
	for(reg int j = 1; j <= n ; ++j) dis[j] = opposite_dis[j] = inf;
	
	q.push(s);reg int u;dis[s] = 0;
	...
	for(reg int j = 1; j <= n ; ++j) ans = ans < dis[j] + opposite_dis[j] ? dis[j] + opposite_dis[j] : ans;
