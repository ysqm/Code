...
int n,k,s,t,head[V + 42],tot_edge = 1;
...
inline int f(const int i,const int j){return (i - 1) * n + j;}

inline void INIT()
{
	n = Read();k = Read();int nn = n * n;s = 1;t = nn << 1;
			
	for(reg int i = 1; i <= n ; ++i)
		for(reg int j = 1; j <= n ; ++j)
		{	
			add_edge(f(i,j),f(i,j) + nn,1,-Read());
			add_edge(f(i,j),f(i,j) + nn,k-1,0);
			
			if(i != n) add_edge(f(i,j) + nn,f(i+1,j),k,0);
			if(j != n) add_edge(f(i,j) + nn,f(i,j+1),k,0);
